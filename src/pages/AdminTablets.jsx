  ...
  return (
    <div className="min-h-screen bg-[#0E0C09] text-[#E2DED0]">
      ...
      <div className="max-w-5xl mx-auto px-8 py-16">
        <div className="label-caps text-[#3F8A66] text-[9px] tracking-[0.3em] mb-3">Corpus</div>
        <h1 className="font-heading text-4xl font-light mb-10">All Tablets</h1>

        <div className="flex flex-wrap gap-3 mb-8">
          {['all', 'published', 'draft'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`label-caps text-[9px] tracking-[0.15em] px-4 py-1.5 border transition-all duration-200 ${filter === f ? 'border-[#3F8A66] text-[#3F8A66]' : 'border-[#2A2620] text-[#3A3530] hover:border-[#3A3530]'}`}
            >
          ...
          {[{ slug: 'all', label: 'All Halls' }, ...halls.map(h => ({ slug: h.slug, label: h.name }))].map(f => (
            <button key={f.slug} onClick={() => setHallFilter(f.slug)}
              className={`label-caps text-[9px] tracking-[0.15em] px-4 py-1.5 border transition-all duration-200 ${hallFilter === f.slug ? 'border-[#3F8A66] text-[#3F8A66]' : 'border-[#2A2620] text-[#3A3530] hover:border-[#3A3530]'}`}
            >
              {f.slug !== 'all' && <span className="font-heading text-[#3F8A66] mr-1">{HALL_LETTERS[f.slug]}</span>}
              {f.label}
            </button>
          ))}
        </div>
              ...
              return (
                <div key={ch.id} className="flex items-center justify-between py-3.5 border-b border-[#1A1815] group">
                  <div className="flex items-center gap-4 min-w-0">
                    {h && (
                      <span className="font-heading text-lg text-[#2A2620] group-hover:text-[#3F8A66] transition-colors shrink-0" style={{ fontWeight: 300 }}>
                  ...
                  <div className="flex items-center gap-3 ml-4 shrink-0">
                    <span className={`label-caps text-[8px] px-2 py-0.5 border hidden sm:block ${ch.status === 'published' ? 'border-[#3F8A66]/30 text-[#3F8A66]' : 'border-[#2A2620] text-[#3A3530]'}`}>
                      {ch.status || 'draft'}
                    </span>
                    ...
                    <Link to={`/admin/tablets/${ch.id}/edit`} className="text-[#3A3530] hover:text-[#3F8A66] transition-colors opacity-0 group-hover:opacity-100">
                      <Pencil size={13} />
                    </Link>
