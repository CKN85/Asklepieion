...
function CrossRefPicker({ onAdd, excludeId }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        // All tablets, not just published — you often want to link a tablet
        // that's still a draft, and it'll be live by the time readers see it.
        const tablets = await base44.entities.Tablet.list('-updated_date', 200);
  ...
  const selectTablet = (ch) => {
    const hall = hallMap.current[ch.hall_id];
    onAdd({
      tablet_id: ch.id,
      tablet_title: ch.title,
      hall_name: hall?.name || '',
      label: ch.title,
    });
    setSearch('');
...
export default function AdminTabletEditor() {
  const { tabletId } = useParams();
  const navigate = useNavigate();
  // The "new tablet" route carries no id at all, so both cases mean "new".
  const isNew = !tabletId || tabletId === 'new';
  ...
  useEffect(() => {
    if (isNew) return;
    setLoading(true);
    base44.entities.Tablet.get(tabletId)
      .then(ch => {
        setForm({
          title: ch.title || '',
          hall_id: ch.hall_id || '',
      ...
      .catch(() => setError('Could not load this tablet.'))
      .finally(() => setLoading(false));
  }, [tabletId, isNew]);

  const chaptersForHall = chapters
    .filter(s => s.hall_id === form.hall_id)
    .sort((a, b) => (a.order || 0) - (b.order || 0));

  const handleSave = async (publishOverride) => {
    if (!form.title.trim()) {
      setError('A tablet needs a title before it can be saved.');
      return;
    }
    setSaving(true);
    setError('');
    try {
      ...
      if (isNew) {
        const created = await base44.entities.Tablet.create(payload);
        navigate(`/admin/tablets/${created.id}/edit`, { replace: true });
      } else {
        await base44.entities.Tablet.update(tabletId, payload);
      }
    ...
    <div className="min-h-screen bg-[#0E0C09] text-[#E2DED0] flex flex-col">
      <div className="bg-[#0E0C09] border-b border-[#1A1815] px-6 h-14 flex items-center justify-between sticky top-0 z-50 shrink-0">
        <div className="flex items-center gap-4">
          <Link to="/admin/tablets" className="flex items-center gap-2 label-caps text-[#3A3530] hover:text-[#7A7268] text-[9px] tracking-[0.15em] transition-colors">
            <ArrowLeft size={11} /> Tablets
          </Link>
          <span className="text-[#1A1815]">/</span>
          <span className="label-caps text-[#7A7268] text-[9px] tracking-[0.15em] truncate max-w-48">
            {isNew ? 'New Tablet' : (form.title || 'Untitled')}
          </span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto px-8 py-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div>
              <label className="label-caps text-[#3A3530] text-[9px] tracking-[0.2em] block mb-2">Chapter</label>
              <select
              ...
              >
                ...
                {chaptersForHall.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
              </select>
            </div>
            <div>
                ...
                {form.cross_references.map((ref, i) => (
                  <div key={i} className="flex items-start justify-between gap-2 bg-[#1A1815] px-3 py-2">
                    <div>
                      ...
                      <div style={{ fontFamily: 'Source Serif 4, Georgia, serif', fontSize: '0.8rem', color: '#A89880' }}>{ref.label || ref.tablet_title}</div>
                    </div>
