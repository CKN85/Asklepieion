...
export default function AdminChapters() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [chapters, filter, hallFilter, halls]);
  ...
  const togglePublish = async (chapter) => {
    const newStatus = chapter.status === 'published' ? 'draft' : 'published';
    await base44.entities.Chapter.update(chapter.id, { status: newStatus });
    ...
    <div className="min-h-screen bg-[#0E0C09] text-[#E2DED0]">
      <div className="bg-[#0E0C09] border-b border-[#1A1815] px-8 h-14 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <span className="label-caps text-[#7A7268] text-[9px] tracking-[0.15em]">Chapters</span>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-8 py-16">
        ...
        <h1 className="font-heading text-4xl font-light mb-10">All Chapters</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            <p className="label-caps text-[#2A2620] tracking-widest">No chapters found</p>
          </div>
        ) : (
          <div className="flex flex-col gap-1">
            {filtered.map(ch => {
              const h = hallMap[ch.hall_id];
              const s = sectionMap[ch.section_id];
              return (
                <div key={ch.id} className="flex items-center justify-between py-3.5 border-b border-[#1A1815] group">
                  ...
                  <div className="flex items-center gap-3 ml-4 shrink-0">
                    ...
                    <Link to={`/admin/chapters/${ch.id}/edit`} className="text-[#3A3530] hover:text-[#C9A84C] transition-colors opacity-0 group-hover:opacity-100">
                      <Pencil size={13} />
                    </Link>
                    <button onClick={() => deleteChapter(ch.id)} className="text-[#3A3530] hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
