...
export default function ChapterPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setSection(null);
      try {
        const ch = await base44.entities.Chapter.get(chapterId);
        setChapter(ch);
        if (ch.hall_id) setHall(await base44.entities.Hall.get(ch.hall_id));
        if (ch.section_id) setSection(await base44.entities.Section.get(ch.section_id));
      } catch (err) {
        console.error(err);
        setChapter(null);
      } finally {
  ...
  useEffect(() => {
    document.title = chapter ? `${chapter.title} — Asklepieion` : 'Asklepieion';
  }, [chapter]);
  ...
  if (!chapter) return (
    <div className="min-h-screen bg-[#0E0C09] flex flex-col items-center justify-center gap-4">
      ...
      <p className="label-caps text-[#3A3530] tracking-widest">Chapter not found</p>
  ...
  const hasRelated =
    (chapter.cross_references?.length > 0) || (chapter.related_reading?.length > 0);
  ...
  return (
    <div className="min-h-screen bg-[#0E0C09] text-[#E2DED0]">
        ...
        <div className="flex items-center gap-3">
          ...
          {section && (
            <>
              ...
              <span className="label-caps text-[#3A3530] text-[9px] tracking-[0.15em]">{section.title}</span>
            </>
          )}
        </div>
      </div>
        ...
        <div className="max-w-3xl">
          ...
          <div className="flex items-center gap-6">
            ...
            {chapter.cross_references?.length > 0 && (
              <div className="flex items-center gap-2 label-caps text-[#3A3530] text-[9px] tracking-[0.15em]">
                <BookOpen size={10} />
                {chapter.cross_references.length} cross-references
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-screen-xl mx-auto px-8 py-16 flex gap-12">
        <article className="flex-1 min-w-0">
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: chapter.body || '<p style="color:#3A3530;font-style:italic">This chapter has no content yet.</p>',
            }}
          />
          ...
          {hasRelated && (
            <div className="xl:hidden mt-16 pt-8 border-t border-[#1A1815]">
              ...
              <div className="flex flex-col gap-3">
                {[...(chapter.related_reading || []), ...(chapter.cross_references || [])].map((ref, i) => (
                  <Link
                    key={i}
        ...
        <RelatedReadingPanel
          crossReferences={chapter.cross_references}
          relatedReading={chapter.related_reading}
        />
      </div>
